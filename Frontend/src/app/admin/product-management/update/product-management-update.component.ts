...
export class ProductManagementUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.productId != null) {
      ...
      //   const myFile = new File([i], 'image', {
      //   type: i.type,
      //  });
      //  this.files.push(myFile)
      // })

      this.productService.load(this.productId).subscribe(blobs => {
        blobs.forEach((blob, index) => {
          const fileName = `image_${index + 1}`; // Đặt tên file theo thứ tự
          const myFile = new File([blob], fileName, { type: blob.type });
          this.files.push(myFile); // Thêm vào danh sách files
        });
      });
    }
  ...
  get productImage(): string {
    return `http://127.0.0.1:8000/api/admin/products/${this.productId}/product-image`;
  }
